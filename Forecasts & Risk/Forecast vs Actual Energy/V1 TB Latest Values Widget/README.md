...
> **v3.0 — Updated 2026-05-14**  
> Default `viewMode` changed to **`ytd_weekly`** — 52 weekly data points with month-name labels (Jan, Feb, ...) on x-axis. Gives 7-day sensitivity with monthly context. Today's partial actual generation included in current week. P95 throughout. Forecast telemetry generated by `pvalue_job.py`.
...
|---|---|---|
| `ytd_weekly` **(default)** | `forecast_p50_weekly` + actual weekly telemetry present | 52 weekly data points for current year. X-axis shows month names at month boundaries. Actual line stops at current week. Today's partial added to current week via `active_power` agg=SUM. |
| `monthly` | Set `viewMode = "monthly"` | 12 monthly bars. Monthly P50/P90/P95 from `forecast_p*_monthly`. Actual from `actual_daily_energy_kwh` + today's partial. |
| `mtd_daily` | Set `viewMode = "mtd_daily"` | Daily points for current month only. |
| `daily` | Set `viewMode = "daily"` | Rolling `windowDays` of daily data. |
...
| `viewMode` | `ytd_weekly` | **Primary**: `ytd_weekly` (52 weekly points, month labels), `monthly`, `mtd_daily`, `daily` |
| `windowDays` | `365` | Rolling window for legacy `daily` mode only |
| `actualEnergyKey` | `active_power` | Realtime kW key for today's partial generation |
| `actualDailyEnergyKey` | `actual_daily_energy_kwh` | Pre-computed daily kWh from daily_job.py |
...
| `forecastP50MonthlyKey` | `forecast_p50_monthly` | P50 monthly forecast key (MWh) |
| `forecastP90MonthlyKey` | `forecast_p90_monthly` | P90 monthly forecast key (MWh) |
| `forecastP95MonthlyKey` | `forecast_p95_monthly` | P95 monthly forecast key (MWh) |
